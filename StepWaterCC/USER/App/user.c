...
for(;;)
		{
          ...
          HeatPidBuf[i].SetPoint = Coldw.T_set; //基本上多余
			    PID_Calc(PidParam, &HeatPidBuf[i], ( wAdcResoult [ i ] - HeatPidBuf[i].SetPoint )); //一般是error = SetPoint - NewPoint ,这里反过来
     ...
     for ( i = 0 ; i < MAX_TEMPRATURE_CHNALL ; i++ )
            {
				    ...
				    if( ( wAdcResoult [ i ] -  Coldw.T_set ) > 15  )
				    	  {//超
				    		...
				    		if( Coldw.Counter_MaxOpen[i] < 12 )				//温度超过设定值15度以上，不使用PID,强制打开水冷，开n次后暂停，然后等温度到正常范围
				    				{//打开 12 x 205步  2460
				    					
                     //后退200/2400步，2400步在8分频时是300步步进电机的极限行程，开阀门
	                   //StepMotRun1(-200);//MOTOR_BACK_STEP);	
	                   OSSemPend(OSSemMotors,0,&err);	
	                   MotorsRun(i,-205);//后退205步
                     OSSemPost(OSSemMotors);			    					
							...
							{//太低
								if( Coldw.Counter_MaxClose[i]	< 123 )			//温度低于设定值15度以上，不使用PID,强制关闭水冷，关n次后暂停，然后等温度到正常范围
										{//关闭 123 x 20步 2460
										//前进2400步，在8分频时是300步步进电机的极限行程，前进到极限，关闭阀门	
	                   //StepMotRun1( 2400);//MOTOR_FORWORD_STEP);//MOTOR_TEST_STEP);
